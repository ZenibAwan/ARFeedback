...
#region Editor script fields
	...
	public string parseApplicationSignature()
	{
		...
		if (licenseFileInfo.Exists)
		{
			...
			if (rootList.Count != 1)
			{
				Debug.LogError("metaioSDK, MetaioSDKLicense.xml has wrong format");
			}
			else
			{
				...
				if (string.IsNullOrEmpty(signatureKey))
				{
					...
					Debug.LogError("metaioSDK, Missing application signature");
#else
					Debug.Log("metaioSDK, Missing application signature");
#endif
				}
				else
				{
		...
		{
#if (UNITY_IPHONE || UNITY_ANDROID) && !UNITY_EDITOR
					Debug.LogError("metaioSDK, No file MetaioSDKLicense.xml found in StreamingAssets");
#else
					Debug.Log("metaioSDK, No file MetaioSDKLicense.xml found in StreamingAssets");
#endif
		}
	...
	public void writeApplicationSignature(string signatureKey)
	{
		...
		{
			Debug.LogError("metaioSDK, Failed to write MetaioSDKLicense.xml ("+signatureKey+"): " + e);
		}
#endif
	}
	...
	void Start ()
	{
		...
		if (result == 0)
		{
			Debug.Log("metaioSDK.Start(), metaioSDK created successfully");
		}
		else
		{
			Debug.LogError("metaioSDK.Start(), Failed to create metaio SDK!");
		}
		...
		{
			Debug.Log("metaioSDK.Start(), No tracking configuration specified");
		...
		else
		{
			...
			if (result == 0)
			{
				Debug.LogError("metaioSDK.Start(), failed to load tracking configuration: "+trackingConfiguration);
			}
			else
			{
				Debug.Log("metaioSDK.Start(), Loaded tracking configuration: "+trackingConfiguration);
			}
		}
