...
#region DLL functions
	...
	[DllImport(METAIO_DLL, CallingConvention=CallingConvention.Cdecl)]
	private static extern int setHandEyeCalibrationByDeviceNative();

	/// <summary>
	/// Sets the hand-eye calibration for stereo rendering from the default file path as created by Metaio Toolbox
	/// </summary>
	/// <returns>
	/// True if a calibration was set, false if nothing was changed because device is unknown
	/// </returns>
	...
	[DllImport(METAIO_DLL, CallingConvention=CallingConvention.Cdecl)]
	private static extern int setHandEyeCalibrationFromFileNative(IntPtr calibrationFilePathUTF8);

	/// <summary>
	/// Sets the hand-eye calibration for stereo rendering from the default file path as created by Metaio Toolbox
	/// </summary>
	/// <returns>
	/// True if a calibration was set, false if nothing was changed because file not existing or invalid
	/// </returns>
	...
	/// <summary>
	/// Sets the hand-eye calibration for stereo rendering from an XML file as created by Metaio Toolbox
	/// </summary>
	/// <returns>
	/// True if a calibration was set, false if nothing was changed because file not existing or invalid
	/// </returns>
	/// <param name="calibrationFilePath">
	/// XML file path. If empty, the method tries to read the default file path as created in Toolbox.
	/// This way, you can allow your users to use Toolbox in order to create an exact calibration.
	/// </param>
	public static bool setHandEyeCalibrationFromFile(string calibrationFilePath)
	{
		...
		byte[] filePathUTF8 = calibrationFilePath.MarshalToZeroTerminatedUTF8Array();
		GCHandle arrayHandle = GCHandle.Alloc(filePathUTF8, GCHandleType.Pinned);
		IntPtr ptr = arrayHandle.AddrOfPinnedObject();
		
		int res = setHandEyeCalibrationFromFileNative(ptr);
